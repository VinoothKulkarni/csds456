...
df_twitter_merged = pd.read_csv("twitter_new_latlong.csv")
df_fs_merged = pd.read_csv("fs_new_latlong.csv")
# df_fs_merged = df_fs_merged.drop_duplicates()
# print("fsmerge",df_fs_merged.shape)
# df_twitter_merged = df_twitter_merged.drop_duplicates()

# print("twittermerge",df_twitter_merged.shape)

# df_twitter_merged.to_csv("twitter_new_latlong.csv")
# df_fs_merged.to_csv("fs_new_latlong.csv")
...
#     if geo_loc["latitude"][]:
#         print(geo_loc["location"])
#         continue
#     else:
#         df.to_csv("fs_lat_lon_new.csv",geo_loc["location"],geo_loc["latitude"],geo_loc["longitude"])
    # latlong[str(geo_loc["location"])] = [geo_loc["latitude"],geo_loc["longitude"]] 
# print(latlong)

# for i,r in df.iterrows():
#     print(i,r)
#     if pd.isna(df[i]['location']):
...
# print(df["location"],latlong[df["location"]].key())
# df.to_csv("new_fs_27.csv")      
...
for index,row in df_twitter_merged.iterrows():
    for index_1,row_1 in df_fs_merged.iterrows():
        ...
        # print(dist_mat,len(dist_mat),dist_mat.shape)
        
np.savetxt('dist.csv', dist_mat, delimiter=",")    
    # exit()